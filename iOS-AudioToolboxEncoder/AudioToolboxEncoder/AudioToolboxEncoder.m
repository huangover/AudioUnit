...
- (void) setupEncoderWithSampleRate:(NSInteger) inputSampleRate channels:(int) channels bitRate:(UInt32) bitRate {
    ...
    AudioConverterGetProperty(_audioConverter,
                              kAudioConverterPropertyMaximumOutputPacketSize,
                              &size, // size of the property "kAudioConverterPropertyMaximumOutputPacketSize", property is UInt32, 这里可以替换成sizeof(UInt32)
                              &_aacBufferSize); // 这个就是描述中的"a UInt32 that indicates the size in bytes of the largest signle packet of data...."
    NSLog(@"Expected BitRate is %@, Output PacketSize is %d", @(bitRate), _aacBufferSize);
//    _aacBufferSize = 1024;
    _aacBuffer = malloc(_aacBufferSize * sizeof(uint8_t)); // 因为_aacBufferSize代表的多少个bytes，uint8_t就是1byte
